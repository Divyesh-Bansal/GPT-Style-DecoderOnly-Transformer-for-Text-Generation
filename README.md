...
A GPT-style decoder-only transformer built from scratch in PyTorch and trained on the tiny-shakespeare dataset for character-level text generation.
...
## Data
The transformer was trained on the `tiny-shakespeare` dataset containing 40,000 lines of text from Shakespeare's plays. Click [here](https://raw.githubusercontent.com/karpathy/char-rnn/master/data/tinyshakespeare/input.txt) for the dataset.
...
$X =$ Matrix containing input embedding vectors → `(context_length, embedding_len)`
...
Note: This implementation makes use of a slightly modified version of this step wherein the attention block's input matrix undergoes normalisation, the attention matrix is computed using this normalised input, and finally, the residual computation is performed. This is known as **pre-normalisation** and is simply a rearrangement of the aforementioned order of steps as follows:
...
### Decoder
This is a decoder-only transformer, similar to GPT.
...
This ensures that the softmax operation assigns zero probability to future tokens while computing attention.

---

### Linear Layer
The decoder output is passed through a final linear layer which maps each token's embedding to the vocabulary space, producing logits for the next token prediction.
...
## Project Structure
```
├── transformer.py          # Full model implementation (embedding, attention, decoder, training, generation)
...
## Usage
1. Run `transformer.py` in a Colab/Jupyter environment (uses `!wget` for dataset download)
2. Training produces checkpoints saved in `models/`
3. Text generation loads a checkpoint and produces Shakespeare-style text
